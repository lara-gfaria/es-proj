...
const PARTICIPATION_COLUMNS_WITH_REVIEWS = "participation (id, acceptance_date, member_rating, member_review, activity_id, volunteer_id, volunteer_rating, volunteer_review)"
...
Cypress.Commands.add('createDatabaseInfoForActivitySuggestion', () => {
  ...
  cy.task('queryDatabase',  {
    query: "INSERT INTO " + USER_COLUMNS + generateUserTuple(8, "VOLUNTEER","DEMO-VOLUNTEER", "VOLUNTEER", "NULL"),
    credentials: credentials,
  })
  cy.task('queryDatabase',  {
    query: "INSERT INTO " + AUTH_USERS_COLUMNS + generateAuthUserTuple(8, "DEMO", "demo-volunteer", 8),
    credentials: credentials,
  })
  cy.task('queryDatabase',  {
    query: "INSERT INTO " + USER_COLUMNS + generateUserTuple(9, "MEMBER","DEMO-MEMBER", "MEMBER", 1),
    credentials: credentials,
  })
  cy.task('queryDatabase',  {
    query: "INSERT INTO " + AUTH_USERS_COLUMNS + generateAuthUserTuple(9, "DEMO", "demo-member", 9),
    credentials: credentials,
  })
  cy.task('queryDatabase',  {
    query: "INSERT INTO " + ACTIVITY_SUGGESTION_COLUMNS + generateActivitySuggestionTuple(2, "2025-03-23 21:59:00", "2025-03-15 21:58:31.345143", "Concerto solidário", "2025-03-25 21:01:00", "Cãocerto Solidário", 5, "2025-03-24 22:00:00", 1, 8),
    credentials: credentials,
  })
  cy.task('queryDatabase',  {
    query: "INSERT INTO " + ACTIVITY_SUGGESTION_COLUMNS + generateActivitySuggestionTuple(3, "2025-03-24 22:00:00", "2025-03-15 22:00:27.167256", "Leilão solidário para angariar fundos", "2025-03-28 23:00:00", "Leilão de Latidos", 3, "2025-03-28 22:00:00", 1, 8),
    credentials: credentials,
  })
});
...
Cypress.Commands.add('createDatabaseInfoForVolunteerProfile', () => {
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ACTIVITY_COLUMNS + generateActivityTuple(1, "A1", "Activity1 description", 
      dayBeforeYesterday.toISOString(), yesterday.toISOString(), yesterday.toISOString(),
    1, 1),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ACTIVITY_COLUMNS + generateActivityTuple(2, "A2", "Activity2 description", 
      dayBeforeYesterday.toISOString(), yesterday.toISOString(), yesterday.toISOString(),
    1, 1),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + PARTICIPATION_COLUMNS_WITH_REVIEWS + generateParticipationWithReviewsTuple(1, 1, 3),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + PARTICIPATION_COLUMNS_WITH_REVIEWS + generateParticipationWithReviewsTuple(2, 2, 3),
    credentials: credentials,
  })
});
...
function generateActivitySuggestionTuple(id, deadline, creation, description, end, name, participants, start, institutionId, volunteerId) {
  return "VALUES ('"
    + id + "', '"
    + deadline + "', '"
    + creation + "', '"
...
function generateParticipationWithReviewsTuple(id, activityId, volunteerId) {
  return "VALUES ("
    + id + ", '2024-02-06 18:51:37.595713', '5', 'A great member review!', " +
    activityId + ", " +
    volunteerId + ", '5', 'A great volunteer review!')";
}
